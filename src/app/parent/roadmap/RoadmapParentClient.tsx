...
// ── Labels (zh strings are the originals) ─────────────────────────────────────
...
const TYPE_EN: Record<MilestoneType, string> = {
  exam:        '📝 Exam',
...
const T = {
  en: {
    ...
    yearTitle: (year: number) => `Year ${year}`,
    yearProgress: (done: number, total: number, pct: number) => `${done}/${total} done · ${pct}%`,
    pageTitle: 'Roadmap',
    curriculum: (c: string) => `${c} curriculum`,
    viewOnly: '👁 View only',
    levelTitle: '🎮 Level & XP',
    levelLine: (level: number, xp: number) => `Level ${level} · ${xp} XP`,
    nextLevel: (pct: number) => `${pct}% → next level`,
    progressTitle: '📊 Progress',
    completed: 'Completed', remaining: 'Remaining',
    badgesTitle: '🏅 Badges Earned',
    noteTitle: '💡 Note',
    noteBody: 'The roadmap is managed by your child. You can view progress but not edit it. To discuss anything, reach out via the Communications hub.',
  },
  zh: {
    due: (date: string) => `截止：${date}`,
    yearTitle: (year: number) => `第 ${year} 学年`,
    yearProgress: (done: number, total: number, pct: number) => `${done}/${total} 完成 · ${pct}%`,
    pageTitle: '学习路线图',
    curriculum: (c: string) => `${c}课程`,
    viewOnly: '👁 仅查看',
    levelTitle: '🎮 成长等级',
    levelLine: (level: number, xp: number) => `等级 ${level} · ${xp} XP`,
    nextLevel: (pct: number) => `${pct}% → 下一等级`,
    progressTitle: '📊 完成情况',
    completed: '已完成', remaining: '待完成',
    badgesTitle: '🏅 获得徽章',
    noteTitle: '💡 提示',
    noteBody: '路线图由孩子自行编辑。您可以查看进度，但无法直接修改。如需讨论，请通过沟通中心与孩子联系。',
  },
}

function typeLabels(locale: Locale) {
  return locale === 'zh' ? TYPE_ZH : TYPE_EN
}
    ...
    <div className="flex items-start gap-3 px-4 py-3 rounded-[8px] hover:bg-[var(--bg)]">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-[var(--t300)] mt-0.5">
          {t.due(new Date(m.due_date).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-SG', { year: 'numeric', month: 'long', day: 'numeric' }))}
        </div>
      </div>
    ...
    <div className="mb-4">
      ...
      >
        ...
        <div className="flex-1 text-left">
          <div className="font-display font-bold text-[14px] text-[var(--t900)]">{t.yearTitle(year)}</div>
          <div className="text-[11px] text-[var(--t500)]">{t.yearProgress(done, total, pct)}</div>
        </div>
        {/* Progress pill */}
            ...
            <div key={type} className="mt-3">
              <div className="px-4 mb-1.5">
                <span className="text-[10px] font-bold text-[var(--t300)] uppercase tracking-[.07em]">
                  {typeLabels(locale)[type as MilestoneType] ?? type}
                </span>
              </div>
      ...
      <div className="bg-white border-b border-[var(--border)] px-9 h-14 flex items-center justify-between sticky top-0 z-40">
        <div>
          ...
          <div className="text-[11px] text-[var(--t500)] mt-0.5">
            {[child.displayName, child.school, child.curriculum && t.curriculum(child.curriculum)].filter(Boolean).join(' · ')}
          </div>
        </div>
        <div className="flex items-center gap-2 text-[11px] text-[var(--t500)]">
          <span className="px-2 py-1 bg-[var(--bg)] rounded-[6px] font-semibold text-[var(--t700)]">{t.viewOnly}</span>
        </div>
      </div>
        ...
        <div className="grid gap-5 grid-cols-1 items-start lg:grid-cols-[1fr_280px]">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-[var(--border)] rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.05)] p-4">
              <div className="font-display font-semibold text-[13px] text-[var(--t900)] mb-3">{t.levelTitle}</div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-[var(--t500)]">{t.levelLine(level.level, xp)}</div>
                </div>
              </div>
              ...
              <div className="text-[11px] text-[var(--t300)] mt-1 text-right">{t.nextLevel(level.progress_pct)}</div>
            </div>

            {/* Progress summary */}
            <div className="bg-white border border-[var(--border)] rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.05)] p-4">
              <div className="font-display font-semibold text-[13px] text-[var(--t900)] mb-3">{t.progressTitle}</div>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="text-center p-3 bg-[var(--green-50)] rounded-[8px]">
                  ...
                  <div className="text-[11px] text-[var(--t500)]">{t.completed}</div>
                </div>
                <div className="text-center p-3 bg-[var(--bg)] rounded-[8px]">
                  ...
                  <div className="text-[11px] text-[var(--t500)]">{t.remaining}</div>
                </div>
              </div>
              {/* By type breakdown */}
              {(Object.entries(typeLabels(locale)) as [MilestoneType, string][]).map(([ty, label]) => {
                const n = milestones.filter(m => m.type === ty).length
                if (n === 0) return null
                const done = milestones.filter(m => m.type === ty && m.completed).length
                return (
                  <div key={ty} className="flex items-center justify-between py-1.5 border-b border-[var(--border)] last:border-0">
                    <span className="text-[12px] text-[var(--t700)]">{label}</span>
            ...
            {badges.length > 0 && (
              <div className="bg-white border border-[var(--border)] rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.05)] p-4">
                <div className="font-display font-semibold text-[13px] text-[var(--t900)] mb-3">{t.badgesTitle}</div>
            ...
            <div className="bg-[#FFFBEB] border border-[#FDE68A] rounded-[10px] p-4">
              <div className="text-[12px] font-semibold text-[#B45309] mb-1">{t.noteTitle}</div>
