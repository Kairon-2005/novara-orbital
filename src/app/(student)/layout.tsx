...
const NAV_ITEMS = [
  ...
  { href: '/essays',     label: 'Essays',            group: 'Applications', d: 'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7 M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z' },
...
// Simplified-Chinese labels for the same nav (locale-switchable).
...
export default async function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = NAV_ITEMS.map(item => ({
    href: item.href,
    label: locale === 'zh' ? NAV_ZH[item.href] ?? item.label : item.label,
    group: locale === 'zh' ? GROUP_ZH[item.group] ?? item.group : item.group,
    icon: <Icon d={item.d} />,
  }))
