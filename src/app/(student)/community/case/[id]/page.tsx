...
export default async function SharedCasePage({ params }: { params: { id: string } }) {
  ...
  if (!user) return <p className="p-10 text-red-500">Please log in to view this case.</p>
  ...
  if (!r || r.verification_status !== 'verified' || r.moderation_status !== 'approved') {
    return (
      <div className="page-content max-w-[720px]">
        <p className="text-[14px] text-[var(--t500)] p-10 text-center">This case isn’t available to share.</p>
        <div className="text-center"><Link href="/community" className="text-[var(--blue)] text-[13px] font-semibold">← Back to community</Link></div>
      </div>
    )
  }

  const sections: [string, string | null][] = [
    ['Admission experience', r.admission_experience],
    ['Interview experience', r.interview_experience],
    ['Scholarship experience', r.scholarship_experience],
  ]

  return (
    <div className="page-content max-w-[720px]">
      <Link href="/community" className="text-[13px] text-[var(--blue)] font-semibold">← Community</Link>
      <div className="card p-5 mt-3">
        ...
        {r.activities && <div className="text-[12px] text-[var(--t500)] mt-1"><span className="font-semibold">Activities:</span> {r.activities}</div>}
        ...
        <p className="text-[11px] text-[var(--t300)] mt-6 pt-4 border-t border-[var(--border)]">
          Verified community case · anonymized. Anecdotal single data point — verify against official sources.
        </p>
      </div>
    </div>
  )
